...
/// 估算终端显示宽度 (CJK=2, ASCII=1)
fn dw(s: &str) -> usize {
    s.chars().map(|c| if c.is_ascii() { 1 } else { 2 }).count()
}

/// 分节标题: `─── Title ──...──` (总宽 W)
fn section(title: &str) {
    let prefix = format!("─── {} ", title);
    let pad = W.saturating_sub(dw(&prefix));
    println!("\n{}{}", prefix, "─".repeat(pad));
}
...
fn pct_bar(pct: f64, width: usize) -> String {
    stats::bar_scaled(pct, 100.0, width)
}
...
fn print_frame_stats(result: &AnalysisResult) {
    section("帧耗时统计");
    let hdr = ["最快", "P25", "P50", "P75", "P90", "P95", "P99", "最慢", "均值", "标准差"];
    println!("  {}", hdr.iter().map(|c| pad_l(c, 7)).collect::<Vec<_>>().join("  "));
    println!("  {}", "─".repeat(88));
    let vals = [
        result.min_ms,
    ...
    println!("  {}", vals.iter().map(|v| pad_l(&format!("{:.1}", v), 7)).collect::<Vec<_>>().join("  "));
}
...
fn print_frame_budget(result: &AnalysisResult) {
    ...
    println!("  {}  {:>2}/{:>2} 超标 {:>5.1}%  {}",
        pad_r("60fps (16.7ms)", 14), result.over_60fps, result.frame_count, p60, pct_bar(p60, bw));
    println!("  {}  {:>2}/{:>2} 超标 {:>5.1}%  {}",
        pad_r("30fps (33.3ms)", 14), result.over_30fps, result.frame_count, p30, pct_bar(p30, bw));
    println!("  {}  {:>2}/{:>2} 超标 {:>5.1}%  {}",
        pad_r(&format!("自定义 ({:.0}ms)", result.threshold), 14), result.slow_count, result.frame_count, ps, pct_bar(ps, bw));
}

fn print_pareto(result: &AnalysisResult) {
    section("热点集中度 (Pareto)");
    if result.total_slow_ms > 0.0 {
        let bw = 32;
        let items = [
            ("Top  1", result.top1_self),
            ("Top  3", result.top3_self),
            ("Top  5", result.top5_self),
            ("Top 10", result.top10_self),
        ];
        for (label, ms) in &items {
            let pct = ms / result.total_slow_ms * 100.0;
            println!("  {} {:>10.1} ms ({:>5.1}%)  {}", label, ms, pct, pct_bar(pct, bw));
        }
    }
}
...
fn print_thread_analysis(data: &ParsedData, result: &AnalysisResult) {
    section("线程分析 (按总耗时降序)");
    println!("  {}  {}  {}  {}  {}",
        pad_r("线程名", 18), pad_l("帧数", 4), pad_l("总耗时ms", 10), pad_l("事件数", 8), pad_l("占比", 6));
    println!("  {}", "─".repeat(54));
    for (name, count, total_ms, events) in &result.thread_stats {
        ...
        if is_slow {
            println!("  {}  {:>4}  {:>10.1}  {:>8}  {:>5.1}%",
                pad_r(name, 18), count, total_ms, events, pct);
        }
    }
}
...
fn print_frame_timeline(data: &ParsedData, result: &AnalysisResult) {
    section("帧时间线 (按捕获顺序)");
    println!("  {}  {}  {}  {}  {}  {}  {}",
        pad_l("#", 3), pad_r("线程", 14), pad_l("耗时ms", 8), pad_l("Δ前帧", 8),
        pad_l("事件", 5), pad_r("进度条", 24), pad_l("状态", 6));
    println!("  {}", "─".repeat(80));
    ...
    for (i, f) in data.frames.iter().enumerate() {
        ...
        prev_ms = dur;
        let status = if dur >= result.threshold { "卡顿" }
            else if dur > 33.33 { "30fps" }
            else if dur > 16.67 { "60fps" }
            else { "流畅" };
        let delta_str = if i > 0 { format!("{:+.1}", delta) } else { "—".into() };
        println!("  {:>3}  {}  {:>8.1}  {:>8}  {:>5}  {}  {}",
            i, pad_r(&f.thread_name, 14), dur, delta_str, f.events.len(),
            stats::bar_scaled(dur, max_dur, 24), pad_l(status, 6));
    }
}
...
fn print_slow_frame_details(data: &ParsedData, result: &AnalysisResult) {
    ...
    section("卡顿帧详情 (Top 5 最慢帧)");
    for &fi in slow_sorted.iter().take(5) {
        let frame = &data.frames[fi];
        let dur = result.durations[fi];
        let title = format!("帧 #{fi} ({}) {:.1} ms / {} 事件", frame.thread_name, dur, frame.events.len());
        ...
        println!("  │ {:>8}  {:>5}  {:>4}  {:>7}  {}", "独占ms", "占比", "次数", "均值/次", "函数名 [文件]");
        println!("  │ {}", "─".repeat(60));
...
fn print_cross_frame_hotspots(result: &AnalysisResult) {
    section("跨帧热点函数 (卡顿帧中按总独占时间排序)");
    println!("  {}  {}  {}  {}  {}  {}  {}  {}  {}",
        pad_l("#", 2), pad_l("独占ms", 8), pad_l("占比", 5), pad_r("进度条", 16),
        pad_l("最大ms", 7), pad_l("次数", 5), pad_l("均值ms", 7), pad_r("帧/总", 6), "函数名");
    println!("  {}", "─".repeat(72));
    for (rank, f) in result.all_funcs.iter().take(30).enumerate() {
        ...
        println!("  {:>2}  {:>8.1}  {:>4.1}%  {}  {:>7.1}  {:>5}  {:>7.1}  {}  {}",
            rank + 1, f.total_self, pct, pct_bar(pct, 16), f.max_self, f.call_count, avg,
            pad_r(&frame_str, 6), f.name);
    }
}
...
fn print_module_aggregation(result: &AnalysisResult) {
    section("模块级聚合 (按源文件)");
    println!("  {}  {}  {}  {}  {}  {}",
        pad_l("#", 2), pad_l("独占ms", 8), pad_l("占比", 5), pad_r("进度条", 20),
        pad_l("函数数", 6), "模块");
    println!("  {}", "─".repeat(62));
...
fn print_critical_path(result: &AnalysisResult) {
    if !result.critical_path.is_empty() {
        ...
        println!("  最大调用深度: {}", result.max_depth);
    }
}
...
fn print_suggestions(data: &ParsedData, result: &AnalysisResult) {
    ...
    if let Some(top1) = result.all_funcs.first() {
        let pct = if result.total_slow_ms > 0.0 { top1.total_self / result.total_slow_ms * 100.0 } else { 0.0 };
        if pct > 50.0 {
            println!("  [严重] '{}' 独占 {:.1}% 卡顿时间", top1.name, pct);
            println!("         出现在 {}/{} 卡顿帧，单帧最高 {:.0} ms", top1.frame_count, result.slow_count, top1.max_self);
            println!("         建议: 优先优化此函数，考虑异步化/分帧/并行化/减少计算量");
        } else if pct > 20.0 {
            println!("  [主要] '{}' 占 {:.1}% 卡顿时间", top1.name, pct);
        }
    }
    ...
    for &idx in result.by_call_count_indices.iter().take(3) {
        ...
        if f.call_count > 1000 {
            ...
            println!("         总独占 {:.1} ms — 考虑减少调用次数或批处理", f.total_self);
        }
    }
    ...
    for &(idx, m, s, cv) in result.stability.iter().take(3) {
        ...
        if cv > 1.0 && f.per_frame_self.len() >= 3 {
            println!("  [不稳] '{}' 变异系数 {:.2} (均值 {:.1} ms, 标准差 {:.1} ms)", f.name, cv, m, s);
    ...
    if result.max_depth > 10 {
        println!("  [调用链] 最慢帧最大调用深度 {} 层 — 考虑扁平化逻辑", result.max_depth);
    }
}